...
# Polling intervals
RSS_INTERVAL_SECONDS = int(os.getenv("RSS_INTERVAL_SECONDS", "120"))
...
def security_publish_decision(item: dict[str, Any]) -> tuple[bool, str, int]:
    ...
    if HIGH_SIGNAL_ONLY and not matches_any(STRONG_SECURITY_SIGNAL_PATTERNS, text):
...
async def collect_feed(
    ...
    *,
    ...
    try:
        ...
        log.warning("Feed failed [%s] %s: %s", source, url, exc)
        return []
    ...
    async def rss_loop(self) -> None:
        ...
        try:
            ...
            await self.mark_collector_success(
                "rss", f"{len(articles)} articles + {len(pocs)} PoC entries scanned"
            )
        except Exception as exc:
            log.exception("RSS loop failed: %s", exc)
